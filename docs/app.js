// app.js — OtOjs 前端核心逻辑
// GitHub API 上传、轮询、状态条、复制下载、Prettier 自动化、后台恢复、智能解密入口、本地存储、输入防抖 + 清空远程 input/output
...
async function loadRemote() {
  ...
  try {
    const r = await fetch(u + '?t=' + nowTs(), { cache: 'no-store' });
...
async function submitInput(){
  ...
  try{
    ...
    if($('#autoResume').checked){
      localStorage.setItem('otojs-job', JSON.stringify({
        repo: $('#repo').value.trim(),
        branch: $('#branch').value.trim(),
        path: 'output/output.js',
        start: Date.now()
      }));
    }
    startAutoPoll(true);
...
// ========== 新增：清空远程 input/output（置空并提交） ==========
async function githubUpsert(path, content){
  const api = `${repoBase()}/contents/${path}`;
  const branch = $('#branch').value.trim();
  let sha = '';
  try{
    const head = await fetch(`${api}?ref=${branch}&t=${nowTs()}`, { headers: ghHeaders(), cache: 'no-store' });
    if (head.ok) {
      const j = await head.json();
      sha = j.sha || '';
    }
  ...
  const body = {
    message: `chore: clear ${path} via UI`,
    content: btoa(unescape(encodeURIComponent(content))), // 允许空字符串
    branch,
    sha: sha || undefined
  };
  const res = await fetch(api, {
...
async function clearRemoteFiles(){
  try{
    ghHeaders(); // 提前校验 token
  }catch(e){
    setStatus(e.message);
    return;
  }
...
async function beautify(){
  ...
  try{
    ...
    setStatus('Prettier 失败：'+e.message);
  }
}
